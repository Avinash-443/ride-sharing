...
def cancel_ride():
    try:
        ...
        if user_email not in passenger_emails:
            return jsonify({'error': 'User not part of this ride'}), 403

        if len(passenger_emails) <= 1:
            # Only one person in ride – delete the ride entirely
            ride_ref.delete()
            return jsonify({'success': True, 'message': 'Ride deleted as no passengers left'}), 200
        else:
            # Remove user from passenger_emails
            passenger_emails.remove(user_email)
            current_count = ride_data.get('current_member_count', 1) - 1
            available_seats = ride_data.get('available_seats', 0) + 1
            ...
            return jsonify({'success': True, 'message': 'User removed from ride'}), 200

    except Exception as e:
        print("Error in cancel_ride:", e)
